//tealium universal tag - utag.2047 ut4.0.202506091550, Copyright 2025 Tealium.com Inc. All Rights Reserved.
var jsSHA256 = (function() {
    ...
    var jsSHABase = function() {
        function class_1(variant, inputFormat, options) {
        ...
        class_1.prototype.update = function(srcString) {
            var i, updateProcessedLen = 0;
            var variantBlockIntInc = this.variantBlockSize >>> 5
        ...
        ;
        class_1.prototype.getHash = function(format, options) {
    ...
    function getNewState256() {
        return H_full.slice();
    }
    function roundSHA256(block, H) {
        var a, b, c, d, e, f, g, h, T1, T2, t;
        var W = [];
        a = H[0];
        b = H[1];
        c = H[2];
    ...
    function finalizeSHA256(remainder, remainderBinLen, processedBinLen, H) {
        var i;
        var offset = (remainderBinLen + 65 >>> 9 << 4) + 15
          , binaryStringInc = 16
    ...
    var jsSHA = function(_super) {
        __extends(class_2, _super);
        function class_2() {
            var _this = _super.call(this, "SHA-256", "TEXT", {
                encoding: "UTF8"
            }) || this;
            _this.bigEndianMod = -1;
            _this.converterFunc = getStrConverter("TEXT", "UTF8", _this.bigEndianMod);
            _this.roundFunc = roundSHA256;
            _this.stateCloneFunc = function(state) {
                return state.slice();
            }
            ;
            _this.newStateFunc = getNewState256;
            _this.finalizeFunc = function(remainder, remainderBinLen, processedBinLen, H) {
                return finalizeSHA256(remainder, remainderBinLen, processedBinLen, H);
            }
            ;
            _this.intermediateState = getNewState256();
            _this.variantBlockSize = 512;
            _this.outputBinLen = 256;
...
try {
    (function(id, loader) {
        ...
        u.map = {
            ...
            "symProdIDs": "prodid",
            "symQuantity": "qty",
            "symProductPrice": "regunitpr",
            "symDiscountPrice": "disunitpr",
            "customer_email_sha256": "hashed_email",
        ...
        u.send = function(a, b) {
            if (u.ev[a] || u.ev.all !== undefined) {
                ...
                u.data = {
                    ...
                    "qs_delim": "" || "?",
                    "tag_type": "img",
                    "base_url": "https://event.symbiosys.dev/orderjs",
                    "secure_base_url": "",
                    "static_params": "api_key=AIzaSyDWKcAd0HdAqT0_Mv810AsRDVOlBYVeV-E",
                    "cachebust": "disabled",
                    "cachevar": "" || "_rnd",
                ...
                if (u.data.static_params) {
                    u.data.static_params.split('&').forEach(function(p) {
                        qs.push(p);
                    });
                }
                for (var key in u.data) {
                    if (!u.data.hasOwnProperty(key))
                        continue;
                    var val = u.data[key];
                    if (['base_url', 'qs_delim', 'qsp_delim', 'kvp_delim', 'tag_type', 'static_params', 'cachebust', 'cachevar', 'requestscriptonce', 'secure_base_url'].indexOf(key) > -1)
                        continue;
                    if (val != null && val !== '') {
                        qs.push(key + u.data.kvp_delim + encodeURIComponent(val));
                    }
                }
                var url = u.data.base_url + u.data.qs_delim + qs.join(u.data.qsp_delim);
